...
int main() {
    ...
    while (1) {
        read_fd = open(MULTIPLIER_PIPE, O_RDONLY);
        if (read_fd == -1) {
            perror("Failed to open multiplier pipe for reading");
            break;
        }
        ...
        write_fd = open(MULTIPLIER_PIPE, O_WRONLY);
        if (write_fd == -1) {
            perror("Failed to open multiplier pipe for writing");
            break;
        }
